...
let syncRetryCount = 0; // Экспоненциальная задержка
let currentDropdownIndex = -1; // Индекс для навигации с клавиатуры
...
async function startMatrixSync(baseUrl, accessToken) {
    ...
    try {
        ...
        if (res.ok) {
            syncRetryCount = 0; // Сбрасываем счетчик при успехе
            const data = await res.json();
...
function handleSyncRetry(baseUrl, accessToken) {
    // Увеличиваем задержку: 2с, 4с, 8с, 16с... Максимум 60с.
    let delay = Math.min(2000 * Math.pow(2, syncRetryCount), 60000);
    syncRetryCount++;
    document.getElementById('syncText').textContent = `Connection error. Retrying in ${delay/1000}s...`;
    setTimeout(() => startMatrixSync(baseUrl, accessToken), delay);
}
...
function closeDropdown(e) {
    ...
    currentDropdownIndex = -1; // Сбрасываем индекс при закрытии
}
